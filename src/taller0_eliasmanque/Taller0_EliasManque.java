...
public class taller0_eliasmanque {
    ...
    // #################################################################################################//hechizos Jugador//OK
    public static void hechizoJugador(String user) throws FileNotFoundException {
        File fileHechizo = new File("HechizosJugadores.txt");
        File jugadorTxt = new File("Jugadores.txt");
        @SuppressWarnings("resource")
        Scanner arch = new Scanner(fileHechizo);
        @SuppressWarnings("resource")
        Scanner archJugadore = new Scanner(jugadorTxt);
        int valorFor = 0;
        while (archJugadore.hasNext()){
        String[] partes = archJugadore.next().split(",");
        String name = partes[0];
        int numHechizosJugador = Integer.parseInt(partes[6]);
        if (user.equals(name)){
            valorFor = numHechizosJugador;
            break;
            }
        }
        String[] nameHechizos = new String[valorFor];
        while(arch.hasNext()){
            String[] partesArch = arch.next().split(",");
            String nombre = partesArch[0];
            if (nombre.equals(user)){
                for (int i = 1; i < valorFor + 1 ;i++){
                    String hechz = partesArch[i];
                    nameHechizos[i-1] = hechz;
                }
            }
        }
        for (int i = 0; i< nameHechizos.length;i++){
            System.out.print(" - "+nameHechizos[i]);
        }System.out.println("");
    }
    // #################################################################################################//aprenderHechizo//
    public static void aprenderHechizo(String user) throws FileNotFoundException {
        ...
        while(arch.hasNext()){
            ...
            String name = partes[0];
            int exp = Integer.parseInt(partes[7]);
            int cantidadHechizos = Integer.parseInt(partes[6]);
            if (name.equals(user)){
                puntosExp = (exp/1000) - cantidadHechizos;
                break;
            }
        }
        if (puntosExp >= 1){
